...
default_settings = {"GREETING": [default_greeting], "ON": False, "CHANNEL": None, "WHISPER" : False, "BOTS_MSG" : None, "BOTS_ROLE" : None}
...
class Welcome:
    ...
    async def welcomeset_bot_role(self, ctx, role : discord.Role=None):
        ...
        fileIO("data/welcome/settings.json", "save", self.settings)
        await self.bot.say("Bots that join this server will now be put into the {} role".format(role.name))
    ...
    async def member_join(self, member):
        ...
        if not member.bot and self.settings[server.id]["WHISPER"]:
            try:
                await self.bot.send_message(ctx.message.author, msg.format(ctx.message.author, server))
            except:
        ...
        welcome_bot = member.bot and self.settings[server.id]["BOTS_MSG"]
        role_bot = member.bot and self.settings[server.id]["BOTS_ROLE"]
        # we can stop here
        if only_whisper and not welcome_bot:
            return
        ...
        if role_bot:
            try:
                role = discord.utils.get(server.roles,
                        name=self.settings[server.id]["BOTS_ROLE"])
                await self.bot.add_roles(member, role)
            except:
                print('welcome.py: unable to add {} role to {}. Role was deleted, '
                      'network error, or lacking permissions'.format(
                      self.settings[server.id]["BOTS_ROLE"], member))
            else:
                print('welcome.py: added {} role to bot, {}'.format(role, member))
        msg = rand_choice(self.settings[server.id]["GREETING"])
        if welcome_bot:
            msg = self.settings[server.id]["BOTS_MSG"]
    ...
    async def send_testing_msg(self, ctx, bot=False, msg=None):
        ...
        if self.speak_permissions(server):
            ...
            if bot or self.settings[server.id]["WHISPER"] is not True:
                await self.bot.send_message(channel, msg.format(ctx.message.author, server))
        else:
